...
class DuplicateReferencesRemover:Bot
{
 ...
 public static void OneThreadOfPageProcessing()
 {
  ...
  for(int i=thread_index ; i<pages_count ; i+=threads_count)
  {
	...
	mc = Regex.Matches(p.text, "\\[(?:http:|https:)?//" + site_url.Replace(".", "\\.") + "/w(?:iki/|/index\\.php\\?title=)([^\\]\\?\\&\\s]+)\\s+([^\\]]+)\\]", RegexOptions.Singleline);
	if(mc.Count==0) {continue;} //there is no point in processing the page if there are no 'external' links to the same project

	foreach(Match m in mc) //process each 'external' link to the same project
	{
		link_target = m.Groups[1].Value.Replace("_", " ");
		link_text = m.Groups[2].Value;
		if(link_target==link_text) {p.text = p.text.Replace(m.Groups[0].Value, "[[" + link_target + "]]");}
		else {p.text = p.text.Replace(m.Groups[0].Value, "[[" + link_target + "|" + link_text + "]]");}
	}
