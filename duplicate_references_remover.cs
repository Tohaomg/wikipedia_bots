...
class DuplicateReferencesRemover:Bot
{
 ...
 public static void OneThreadOfPageProcessing()
 {
  ...
  MatchCollection mc;
  string first_ref = "", ref_head = "", match_string = "", founded_ref = "";
  int index = 0;
  Regex t_re;
  Page p;
  List<string> ref_bodies;
  ...
  for(int i=thread_index ; i<pages_count ; i+=threads_count)
  {
	...
	if( !p.text.Contains("</ref>") ) {continue;} //there is no point in processing the page if there are no references

	//look for references using a regexp
	mc = Regex.Matches(p.text, "(<ref(?:\\s+name=\"[^\"\\>]+\"\\s*)?>)(.+?)</ref>", RegexOptions.Singleline);
	while(mc.Count!=0)
	{
		ref_head = "";
		match_string = "";
		founded_ref = "";
		ref_bodies = new List<string>();
		foreach(Match m in mc) //iterate over each found reference
		{
			ref_head = m.Groups[1].Value; //store opening tag separately
			match_string = m.Groups[2].Value; //store the content of a reference, without tags
			if(!ref_bodies.Contains(ref_head + match_string)) //if there is no such reference in the list of found references, then add it to the list
			{
				ref_bodies.Add(ref_head + match_string);
			}
			else //if such reference is already present in the list, it means that there are at least two identical references, which should be processed
			{
				founded_ref = ref_head + match_string + "</ref>";
				break; //it has found a reference to work with, no point in iterating further
			}
		}
		if(founded_ref=="") {goto next_page;} //if all references were iterated over and no identical ones were found, go to next page ('goto' is used because 'break' will exit 'while', but not 'for')

		if(ref_head=="<ref>") //if it is an unnamed reference
		{
			index = 0; //it will assign a numeric name to the refence, but for this an unused numeric name should be found
			while( Regex.Matches(p.text, "name\\s*=\\s*\"?\\s*:" + index + "[>\"\\s]").Count!=0 ) {index++;} //increase numeric name while an unused one is reached
			first_ref = founded_ref.Replace("<ref>", "<ref name=\":" + index + "\">"); //put the name to the first reference
			t_re = new Regex(Regex.Escape(founded_ref));
			p.text = t_re.Replace(p.text, first_ref, 1); //add the name only to the first of identical reference
			p.text = p.text.Replace(founded_ref, "<ref name=\":" + index + "\"/>"); //replace second an all following references with a link to the first
		}
		else //if this reference has a name
		{
			t_re = new Regex(Regex.Escape(founded_ref)); //replace only the first of identical references with stub text
			p.text = t_re.Replace(p.text, "~~~~", 1);
			p.text = p.text.Replace(founded_ref, ref_head.Replace(">", "/>")); //replace second an all following references with a link to the first
			p.text = p.text.Replace("~~~~", founded_ref); //now replace stub text back to the text of the reference
		}
		...
		mc = Regex.Matches(p.text, "(<ref(?:\\s+name=\"[^\"]+\"\\s*)?>)(.+?)</ref>", RegexOptions.Singleline); //look for references in the updated text (for the next iteration)
	}

	//point to jump to when it is needed to skip this page
	next_page:
	{}
  }
 }
